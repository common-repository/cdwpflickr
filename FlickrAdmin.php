...
if (! class_exists ( 'FlickrAdmin' )) :
	...
	class FlickrAdmin {
		...
		function CreateMenus() {
			...
			$page = add_options_page ( 'Flickr Options', 'CdWPFlickr', 'manage_options', $flickr_manager->GetBaseName (), array (
					&$this,
					'SettingsPage' 
			) );
...
// 						}
			...
			if (! empty ( $flickr_manager->settings ['is_pro'] ) && $flickr_manager->settings ['is_pro'] == '1')
			...
			$overlays = array_merge ( array (
					__ ( 'Disabled', 'flickr-manager' ) 
			), $overlayLoader->GetOverlays () );
			unset ( $overlayLoader );
		...
		<h2>%s</h2>';
			...
			echo sprintf ( $header, admin_url ( '/options-general.php?page=' . $flickr_manager->GetBaseName () ), __ ( 'Flickr Manager Settings', 'flickr-manager' ) );
			
			wp_nonce_field ( 'wfm_settings_page' );
			
// 			if (! $flickr_manager->flickr->auth_checkToken ()) {
			if (! $flickr_manager->flickr->oauth_checkToken ()) {
				$this->LoginPage ();
				return;
			}
			...
			echo $this->RenderPostboxColumn ( array (
					array (
							...
							'content' => $this->RenderFormTable ( array (
									array (
			...
			$info = $flickr_manager->flickr->people_getInfo (urldecode( $flickr_manager->settings ['nsid'] ));
			if ($info ['ispro'] != 0) {
					...
					array (
							...
							'content' => $this->RenderFormTable ( array (
									...
									array (
											...
											'type' => 'custom',
											'param' => $info ['realname'] ['_content']
									),
									array (
											'label' => __ ( 'Photo URL', 'flickr-manager' ),
											'type' => 'custom',
											'param' => sprintf ( '<a href="%s">%s</a>', $info ['photosurl']['_content'], $info ['photosurl']['_content'] ) 
									),
									array (
											'label' => __ ( '# Photos', 'flickr-manager' ),
											'type' => 'custom',
											'param' => $info ['photos'] ['count'] ['_content']
									) 
							), false ) . sprintf ( '<div class="alignright"><input class="button-primary logoutButton" type="button" value="%s" /></div>', __ ( 'Logout &raquo;', 'flickr-manager' ) ) 
					),
					array (
							...
							'content' => '<h2 style="margin-top: 0px; padding-top:0px;">Like this plugin?</h2>
								...
								<ul style="list-style-position: outside; list-style: disc; margin: 10px 20px; vertical-align: top; padding-left: 5px;">
										<li>
										<a href="http://www.xidige.com/">Link</a> to my website http://www.xidige.com.
									</li>		
										<li>
										<a target="_blank" href="http://tgardner.net/wordpress-flickr-manager/">Link</a> to it so other folks can find out about it.
									</li>
									<li>
										<a target="_blank" href="http://wordpress.org/extend/plugins/wordpress-flickr-manager/">Give it a good rating</a> on WordPress.org.
									</li>
									<li>
										<a target="_blank" href="https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=cidy0106@gmail.com&lc=US&currency_code=USD&amount=10">
								...
								<h2>Need Support?</h2>
								<p>
									If you have any problems or good ideas, please visit: 
								</p>
								<p>
								<a target="_blank" href="http://www.xidige.com/">www.Xidige.com</a>
								</p>
								<p>
									Or talk about them in the 
		...
		function LoginPage() {
			...
			// oauth1先要request code
			$callbackurl = $_SERVER ['SERVER_NAME'] . $_SERVER ['REQUEST_URI'];
			if (! empty ( $_SERVER ['HTTPS'] ) && $_SERVER ["HTTPS"] == "on") {
				$callbackurl = 'https://' . $callbackurl;
			} else {
				$callbackurl = 'http://' . $callbackurl;
			}
			$requestcode = $flickr_manager->flickr->oauth_requestcode ( $callbackurl );
			$flickr_manager->afterRequestCode ( $requestcode ['oauth_token'], $requestcode ['oauth_token_secret'] );
			
			$tpl_param = array (
					'[StepText]' => __ ( 'Step', 'flickr-manager' ),
					'[AuthText]' => $title,
					// ,'[AuthURL]' => sprintf("http://flickr.com/services/auth/?%s", http_build_query($params))
					'[AuthURL]' => sprintf ( '%s?oauth_token=%s', $flickr_manager->flickr->authorizeUrl, $requestcode ['oauth_token'] ),
		...
		function RenderControl($option) {
			...
			switch ($option ['type']) {
				case 'custom' :
					...
					break;
				case 'checkbox' :
					$result = sprintf ( '<input type="checkbox" name="wfm-%s" id="wfm-%s" value="true" %s />', $option ['name'], $option ['name'], ($flickr_manager->settings [$option ['name']] == 'true') ? 'checked="checked"' : '' );
				...
				case 'select' :
					...
					break;
				case 'textarea' :
					$result = sprintf ( '<textarea name="wfm-%s" id="wfm-%s" style="overflow: auto; height: 100px; width: 250px;">%s</textarea>', $option ['name'], $option ['name'], $flickr_manager->settings [$option ['name']] );
				...
				case 'multiple' :
					...
					break;
				case 'text' :
					$result = sprintf ( '<input type="text" name="wfm-%s" id="wfm-%s" value="%s" style="%s" />', $option ['name'], $option ['name'], $flickr_manager->settings [$option ['name']], $option ['style'] );
